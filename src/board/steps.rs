...
fn path<F>(len: i8, generator: F) -> Vec<usize> where F: Fn(i8) -> Coords {
    let mut result = vec![];
    for d in 1..len + 1 {
...
fn paths(field: usize) -> [Vec<usize>; 4] {
  ...
  [
    path(coords.max_x() - coords.x, |d| Coords { x: coords.x + d, y: coords.y }),
    path(coords.max_y() - coords.y, |d| Coords { x: coords.x, y: coords.y + d }),
    path(coords.x - coords.min_x(), |d| Coords { x: coords.x - d, y: coords.y }),
    path(coords.y - coords.min_y(), |d| Coords { x: coords.x, y: coords.y - d }),
  ]
}
