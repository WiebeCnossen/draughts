...
const COL_POW : [u64; ROW_FIELDS + 1] =
  [ 1, 5, 25, 125, 625, 3125 ];
...
impl Position for CodedPosition {
  fn side_to_move(&self) -> Color {
    if SIDE_BIT & self.lower != 0 { Color::White } else { Color::Black }
  }
...
impl Game for CodedPosition {
  ...
  fn put_piece(&self, field: usize, piece: u8) -> CodedPosition {
    if field < 25 {
      CodedPosition { upper: put_piece(self.upper, field, piece), lower: self.lower }
    }
    else {
      CodedPosition { upper: self.upper, lower: put_piece(self.lower, field - 25, piece) }
    }
  }
}
